...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt, const double timeout)
{
    ...
    while (!openSet_.empty())
    {
        ...
        if (current->index(0) == endPtr->index(0) && current->index(1) == endPtr->index(1) && current->index(2) == endPtr->index(2))
        {
            ...
            printf("Time consume in A star path finding is %.3fs, iter=%d\n", (time_2 - time_1).toSec(), num_iter);
        ...
        ros::Time time_2 = ros::Time::now();
        if ((time_2 - time_1).toSec() > timeout)
        {
            ROS_WARN("Failed in A star path searching !!! %f seconds time limit exceeded.", timeout);
            return false;
        }
    }
